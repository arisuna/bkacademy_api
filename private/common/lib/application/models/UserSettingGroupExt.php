...
class UserSettingGroupExt extends UserSettingGroup
{	
	/** status archived */
    ...
    public function __saveData($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['user_setting_group_id']) && $custom['user_setting_group_id']  > 0 ? $custom['user_setting_group_id'] : $req->getPut('user_setting_group_id');
                if( $data_id > 0 ){

...
class UserSettingDefaultExt extends UserSettingDefault
{	
	/** status archived */
	...
	const DISPLAY_LANGUAGE = 'display_language';
    ...
    public function __saveData($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['user_setting_default_id']) && $custom['user_setting_default_id']  > 0 ? $custom['user_setting_default_id'] : $req->getPut('user_setting_default_id');
                if( $data_id > 0 ){

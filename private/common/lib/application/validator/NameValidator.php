...
class NameValidator extends Validator implements ValidatorInterface
{
    /**
    ...
    public function validate(\Phalcon\Validation $validator, $attribute)
    {
        ...
        $dot = (bool)$this->getOption('dot');
        $dot = $dot ? '\.' : '';

        if (!preg_match('/^([\p{L}0-9' . $whiteSpace . $underscore . $hyphen . $dot . '])+$/u', $value)) {
            $message = $this->getOption('message');
            if (!$message) {
                if ($whiteSpace && $underscore && $dot) {
                    $message = 'The value can contain only alphanumeric, underscore, dot and white space characters';
                } elseif ($whiteSpace) {
                    ...
                    $message = 'The value can contain only alphanumeric and dot characters';
                } else {

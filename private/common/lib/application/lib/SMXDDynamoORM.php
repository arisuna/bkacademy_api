...
class SMXDDynamoORM
{
    ...
    protected static $_config = array(self::DEFAULT_CONNECTION => array());
    ...
    public $_with_filter_expression = false;
    public $_with_key_expression = false;
    ...
    protected $_filter_expression = "";
    protected $_expression_attribute_values = "";
    protected $_expression_attribute_names = [];

    protected $_key_condition_expression = "";
    protected $_argsLog = "";
    ...
    public function findMany(array $options = array())
    {
        ...
        if ($this->_with_key_expression) {
            $options['KeyConditionExpression'] = $this->_key_condition_expression;
        } else {
            $conditions = $this->_buildConditions($this->_where_conditions);
            $options['KeyConditions'] = $conditions;
        }

        if ($this->_filter_conditions && $this->_with_filter_expression == false) {
            $filter_conditions = $this->_buildConditions($this->_filter_conditions);
            $options['QueryFilter'] = $filter_conditions;
        } elseif ($this->_filter_expression && $this->_with_filter_expression == true) {
            if (!isset($options['FilterExpression']) || $options['FilterExpression'] == null || $options['FilterExpression'] == "") {
                $_str_expression = substr($this->_filter_expression, 0, 4);
                if ($_str_expression == " AND") {
                    $options['FilterExpression'] = substr_replace($this->_filter_expression, '', 0, 4);
                } else {
                    $options['FilterExpression'] = $this->_filter_expression;
                }
            } else {
                $options['FilterExpression'] .= " AND " . $this->_filter_expression;
            }
        }
        if ($this->_expression_attribute_values != "") {
            $options['ExpressionAttributeValues'] = $this->_expression_attribute_values;
        }

        if ($this->_expression_attribute_names) {
            $options['ExpressionAttributeNames'] = $this->_expression_attribute_names;
        }
    ...
    /**
     * Retrieve first by uuid result using query
     *
     * @param array $options
     *
     * @return $this|null
    ...
    public static function findFirstByUuid($uuid)
    {
        try {
            ...
            $model = (new static)::factory($className)->findOne($uuid);
            return $model;
        } catch (\Exception $e) {
            return $e->getCode();
        }
    ...
    public function deleteAsync()
    {
        ...
        $_client = $this->getClient();
        $result = $_client->deleteItemAsync($args);
        return $result;
    }
     ...
     * @return $this
     */
    public function whereWithExpression($value)
    {
        $this->_with_key_expression = true;
        $this->_key_condition_expression .= $value;
        return $this;
    }
    ...
    public function filter()
    {
        ...
        if (func_num_args() == 2) {
            ...
            $value = $args[2];
            $operator = $this->_convertOperator($args[1]);
            $conditional_operator = isset($args[3]) ? $args[3] : 'AND';
        }

        $this->_filter_conditions[] = array($key, $operator, $value, $conditional_operator);
        return $this;
    }

    /**
     * use FILTER EXPRESSION of AWS SDK
     * clone of filter with Filter Expression
     * @param $value
     * @return $this
     */
    public function filterWithExpression($value)
    {
        $this->_with_filter_expression = true;
        $this->_filter_expression .= $value;
        return $this;
    }
     ...
     *
     */
    public function setExpressionAttributeValues($values)
    {
        $this->_expression_attribute_values = $values;
        return $this;
    }
     ...
     *
     */
    public function setExpressionAttributeNames($attributeNames)
    {
        $this->_expression_attribute_names = $attributeNames;
        return $this;
    }
...
//            'KeyConditions' => $conditions,
        ...
        // Merge $options to $args
        $option_names = array('ScanIndexForward', 'QueryFilter', 'FilterExpression', 'ExpressionAttributeValues', 'ExpressionAttributeNames', 'KeyConditionExpression', 'KeyConditions');
        foreach ($option_names as $option_name) {
            if (isset($options[$option_name])) {
                $args[$option_name] = $options[$option_name];
            }
...
//            die(__METHOD__);
    ...
    public function findAll(array $options = array())
    {
        ...
        if ($this->_filter_expression && $this->_with_filter_expression == true) {
            if (!isset($options['FilterExpression']) || $options['FilterExpression'] == null || $options['FilterExpression'] == "") {
                $_str_expression = substr($this->_filter_expression, 0, 4);
                if ($_str_expression == " AND") {
                    $options['FilterExpression'] = substr_replace($this->_filter_expression, '', 0, 4);
    ...
    public static function factory($class_name, $connection_name = self::DEFAULT_CONNECTION)
    {
        ...
        if (method_exists($object, 'getDefaultTableName')) {
            $object->setTableName($object->getDefaultTableName());
        }
        return $object;
    }
    ...
    public static function __init()
    {
        if (self::__isLocal() == true) {
            self::configure("region", 'us-east-1');
            self::configure("profile", 'default');
            self::configure("base_url", Di::getDefault()->getShared('appConfig')->aws->dynamoEndpointUrl);
            self::configure("endpoint", Di::getDefault()->getShared('appConfig')->aws->dynamoEndpointUrl);
        } else {
            self::configure("region", Di::getDefault()->getShared('appConfig')->aws->region);
        }
    }
     ...
     * @return boolß
     */
    public static function __isLocal()
    {
        if (Di::getDefault()->getShared('appConfig')->aws->dynamoEndpointLocal == true) {
            return true;
        } else {
            return false;
        }
    ...
    public function _buildConditions($conditions)
    {
        ...
        foreach ($conditions as $i => $condition) {
            ...
            $value = $condition[2];
            $condition_operator = isset($condition[3]) ? $condition[3] : 'AND';
            if (!is_array($value)) {
            ...
            foreach ($value as $v) {
                if ($operator != 'IN') {
                    ...
                    if ($this->_getDataType($key) != 'L') {
                        $attributes[] = array($this->_getDataType($key) => $val);
                    } else {
                        $attributes[] = array('S' => $val);
                    }
                } else {
                    $attributes[] = array($this->_getDataType($key) => $v);
                }
            ...
            $result[$key] = array(
                ...
                'ConditionalOperator' => $condition_operator
            );
        }
        return $result;
    }
    ...
    protected static function _setupClient($connection_name = self::DEFAULT_CONNECTION)
    {
        if (!isset(self::$_client[$connection_name])) {
            ...
            if (self::__isLocal() == false) {
                $di = Di::getDefault();
                $client = $di->get('aws')->createDynamoDb();  //new DynamoDbClient($params);
                self::$_client[$connection_name] = $client;
            } else {
                $credentials = new Credentials('key_local', 'secret_local');
    ...
    {
        return $this->_exclusive_start_key;
    }
     ...
     * @return array
     */
    public function getPutItemParameter()
    {
        $values = $this->_compact($this->_data);
        $args = array(
            'Put' => [
     ...
     * @return array
     */
    public function getUpdateItemParameter($array = [])
    {
        $conditions = $this->_getKeyConditions();
        $updateExpression = $this->_buildUpdateExpression($array);
        $args = array(
            'Update' => [
                'TableName' => $this->_table_name,
                'Key' => $conditions,
     ...
     * @return array
     */
    public function getDeleteItemParameter()
    {
        $conditions = $this->_getKeyConditions();
        $args = array(
            'Delete' => [
    ...
    /**
     * Build update conditions expression
     *
     * @param $array
     * @return array $result
     */
    public function _buildUpdateExpression($array = [])
    {
        $args = [
            'ExpressionAttributeNames' => [],
            'ExpressionAttributeValues' => [],
            'UpdateExpression' => 'SET ',
            'ReturnValuesOnConditionCheckFailure' => 'ALL_ODD'
        ];

        $attributes = $this->_formatAttributes($array);
        $num = 1;
        $update_expression = 'v';
        foreach ($this->_schema as $column => $type) {
            if (isset($array[$column])) {
                $args['ExpressionAttributeNames'][] = ["#$update_expression$num" => $column];
                $args['ExpressionAttributeValues'][] = [":$column" => [$attributes[$column]]];
                $args['UpdateExpression'] .= "#$update_expression$num = :$column,";
                $num++;
            }
        }
        $args['UpdateExpression'] = substr_replace($args['UpdateExpression'], "", -1);
     ...
     * @return array
     */
    public static function transactWriteItems($methods = [], $options = [])
    {
        $args = [
            'ReturnConsumedCapacity' => 'NONE',
            'ReturnItemCollectionMetrics' => 'SIZE',
            'TransactItems' => $methods
        ];

        // Merge $options to $args
        $option_names = array('ClientRequestToken', 'ReturnConsumedCapacity', 'ReturnItemCollectionMetrics');
        foreach ($option_names as $option_name) {
            if (isset($options[$option_name])) {
                $args[$option_name] = $options[$option_name];
            }
        ...
        $_client = (new self)->getClient();

        try {
            $item = $_client->transactWriteItems($args);
            return [
                'success' => true,
     ...
     *
     */
    public function getArgsLog(){
        return $this->_argsLog;
    }
}

class SMXDDynamoORMException extends \Exception
{

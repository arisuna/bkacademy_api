...
class NeedFormRequestAnswer extends \Reloday\Application\Models\NeedFormRequestAnswerExt
{	
	...
	public function __save($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['need_form_request_answer_id']) && $custom['need_form_request_answer_id']  > 0 ? $custom['need_form_request_answer_id'] : $req->getPut('need_form_request_answer_id');
                if( $data_id > 0 ){

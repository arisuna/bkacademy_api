...
class AppSettingDefault extends \Reloday\Application\Models\AppSettingDefaultExt
{
    ...
    {
        parent::initialize();
        $this->hasMany('id', 'Reloday\Gms\Models\AppSetting', 'app_setting_default_id', [
            'alias' => 'AppSetting'
        ]);
    }
    ...
    public function __save($custom = [])
    {
        ...
        if (!($model->getId() > 0)) {
            if ($req->isPut()) {
                $data_id = isset($custom['app_setting_default_id']) && $custom['app_setting_default_id'] > 0 ? $custom['app_setting_default_id'] : $req->getPut('app_setting_default_id');
                if ($data_id > 0) {
     ...
     * @return mixed
     */
    public function getValue($app_id = 0)
    {
        if ($app_id == 0) $app_id = ModuleModel::$app->getId();
        $appSettingApp = $this->getAppSetting([
        ...
        if ($appSettingApp && $appSettingApp->count() > 0) {
            return $appSettingApp->getFirst()->getValue();
        }
    }

...
class OldCommunicationTopicFollower extends \Reloday\Application\Models\OldCommunicationTopicFollowerExt
{	
	...
	public function initialize(){
		parent::initialize();
		$this->belongsTo('user_profile_id','Reloday\Gms\Models\UserProfile','id',['alias' => 'UserProfile','reusable' => true ]);
	}
	...
	public function __save($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['communication_topic_follower_id']) && $custom['communication_topic_follower_id']  > 0 ? $custom['communication_topic_follower_id'] : $req->getPut('communication_topic_follower_id');
                if( $data_id > 0 ){

...
class CommunicationTopicItem extends \Reloday\Application\Models\CommunicationTopicItemExt
{
    ...
    public function initialize()
    {
        ...
        $this->belongsTo('sender_user_profile_id', 'Reloday\Gms\Models\UserProfile', 'id', [
            ...
            'reusable' => true,
            'cache' => [
                'key' => 'USER_PROFILE_' . $this->getSenderUserProfileId(),
                'lifetime' => CacheHelper::__TIME_6_MONTHS
            ],
        ]);
        $this->hasMany('uuid', 'Reloday\Gms\Models\CommunicationTopicRead', 'topic_uuid', [
    ...
    public function findWithFilter($options)
    {
        ...
        $queryBuilder->addFrom('\Reloday\Gms\Models\CommunicationTopicItem', 'CommunicationTopicItem');
    ...
    public function isUserRead($user_profile_id = 0)
    {
        ...
        $readTopicManager = CommunicationTopicRead::findTopicUser($this->getUuid(), $user_profile_id);
        if ($readTopicManager) {
            return true;
        } else {
            return false;
    ...
    {
        $readTopicManager = CommunicationTopicRead::findTopicUser($this->getUuid(), ModuleModel::$user_profile->getId());
        if (!$readTopicManager) {
            $readTopicManager = new CommunicationTopicRead();
            $readTopicManager = $readTopicManager->__create([
                'topic_uuid' => $this->getUuid(),
                'user_profile_id' => ModuleModel::$user_profile->getId(),
            ]);
            if ($readTopicManager instanceof CommunicationTopicRead) {
                return true;
            } else {
                return false;
            }

...
class UserProfileData extends \Reloday\Application\Models\UserProfileDataExt
{	
	...
	public function __save($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['user_profile_data_id']) && $custom['user_profile_data_id']  > 0 ? $custom['user_profile_data_id'] : $req->getPut('user_profile_data_id');
                if( $data_id > 0 ){

...
class EmailTemplate extends \Reloday\Application\Models\EmailTemplateExt
{	
	...
	public function __save($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['email_template_id']) && $custom['email_template_id']  > 0 ? $custom['email_template_id'] : $req->getPut('email_template_id');
                if( $data_id > 0 ){

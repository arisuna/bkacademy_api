...
class CompanyData extends \Reloday\Application\Models\CompanyDataExt
{	
	...
	public function __save($custom = []){
        ...
        if( !($model->getId() > 0) ){
            if ($req->isPut()) {
                $data_id = isset($custom['company_data_id']) && $custom['company_data_id']  > 0 ? $custom['company_data_id'] : $req->getPut('company_data_id');
                if( $data_id > 0 ){

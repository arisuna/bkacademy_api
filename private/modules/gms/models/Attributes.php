...
class Attributes extends \Reloday\Application\Models\AttributesExt
{
    /**
     * @return \Reloday\Application\Models\Attributes[]
     */
    public static function __getAllAttributes()
    {
        return self::find([
            "conditions" => "is_gms = :is_gms_yes:",
            "bind" => [
                "is_gms_yes" => ModelHelper::YES,
            ],
            "cache" => [
                "key" => "__ALL_ATTRIBUTES_GMS__",
                "lifetime" => 86400,
            ],
        ]);
    }
     ...
     * @return \Reloday\Application\Models\Attributes[]
     */
    public static function __getAllAttributesByCompany($company_id, $language)
    {
        $queryBuilder = new \Phalcon\Mvc\Model\Query\Builder();
        $queryBuilder->addFrom('\Reloday\Gms\Models\Attributes', 'Attributes')
            ->distinct(true)
            ->leftjoin('\Reloday\Gms\Models\AttributesValue', 'AttributesValue.attributes_id = Attributes.id', 'AttributesValue')
            ->leftjoin('\Reloday\Gms\Models\AttributesValueTranslation', 'AttributesValueTranslation.attributes_value_id = AttributesValue.id', 'AttributesValueTranslation')
            ->where('(AttributesValue.standard = 1 OR AttributesValue.company_id = :company_id:) AND AttributesValue.archived = 0', [
                'company_id' => ModuleModel::$company->getId(),
            ])
            ->andWhere('(Attributes.is_gms = :is_gms_yes:)', ['is_gms_yes' => ModelHelper::YES])
            ->andWhere('AttributesValueTranslation.language = :language:', ['language' => $language]);
    ...
    {
        parent::initialize();
        $this->hasMany('id', '\Reloday\Gms\Models\AttributesValue', 'attributes_id', [
            'alias' => 'values',
        ]);
    }

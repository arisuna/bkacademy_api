...
class Media extends MediaExt
{
    ...
    {
        $this->url_token = ApplicationModel::__getApiHostname() . '/media/file/load?uuid=' . $this->getUuid() . '&type=' . $this->getFileType() . "&token=" . base64_encode(ModuleModel::$user_login_token) . "&name=" . urlencode($this->getName() . "." . $this->getFileExtension());
    ...
    {
        $this->url_full = ApplicationModel::__getApiHostname() . '/media/file/full?uuid=' . $this->getUuid() . '&type=' . $this->getFileType() . "&token=" . base64_encode(ModuleModel::$user_login_token) . "&name=" . urlencode($this->getName() . "." . $this->getFileExtension());
    ...
    {
        $this->url_thumb = ApplicationModel::__getApiHostname() . '/media/file/thumbnail?uuid=' . $this->getUuid() . '&type=' . $this->getFileType() . "&token=" . base64_encode(ModuleModel::$user_login_token) . "&name=" . urlencode($this->getName() . "." . $this->getFileExtension());
    ...
    {
        $this->url_download = ApplicationModel::__getApiHostname() . '/media/file/download?uuid=' . $this->getUuid() . '&type=' . $this->getFileType() . "&token=" . base64_encode(ModuleModel::$user_login_token) . "&name=" . urlencode($this->getName() . "." . $this->getFileExtension());
    ...
    public static function __findWithFilters($options = [], $orders = [])
    {
        ...
        if (isset($options['user_profile_uuid']) && Helpers::__isValidUuid($options['user_profile_uuid']) && $options['user_profile_uuid']) {
            $queryBuilder->andwhere("Media.user_profile_uuid = :user_profile_uuid:", [
...
//        }
    ...
    public function getParsedData($items = [])
    {
        ...
        $item['is_employee'] = $this->isEmployee();
...
//        }
    ...
    {
        if ($this && $this->getUserProfileUuid() === ModuleModel::$user_profile->getUuid()) {
            return true;
        } else {
            return false;
        }

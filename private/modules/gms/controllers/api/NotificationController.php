...
use Reloday\Gms\Models\UserReadNotification;
...
class NotificationController extends BaseController
{
    ...
    public function addNotificationAction()
    {
        ...
        $data['creator_user_profile_uuid'] = ModuleModel::$user_profile->getUuid();
        ...
        $dataParams = [
            ...
            'action' => RelodayQueue::ACTION_SEND_NOTIFICATION,
    ...
    public function addNotificationToAccountAction()
    {
        ...
        $dataParams = [
            ...
            'action' => RelodayQueue::ACTION_SEND_NOTIFICATION_TO_ACCOUNT,
            'language' => ModuleModel::$system_language,
            'params' => $data,
        ];
        $resultQueue = $queueSendMail->addQueue($dataParams);
    ...
    public function addNotificationFullAction()
    {
        ...
        if (!isset($members[ModuleModel::$user_profile->getUuid()])) {
            $currentUser = ModuleModel::$user_profile->toArray();
            $currentUser['principal_email'] = ModuleModel::$user_profile->getPrincipalEmail();
            $currentUser['is_current_user'] = true;
            $currentUser['is_owner'] = false;
            $currentUser['is_reporter'] = false;
            $currentUser['is_viewer'] = false;
            $currentUser['is_active'] = ModuleModel::$user_profile->isActive();
            $members[ModuleModel::$user_profile->getUuid()] = $currentUser;
        }
    ...
    public function addNotificationForUserAction()
    {
        ...
        if ($object_uuid != '' && Helpers::__isValidUuid($object_uuid)) {
            ...
            if (!isset($viewers[ModuleModel::$user_profile->getUuid()])) {
                $viewers[ModuleModel::$user_profile->getUuid()] = ModuleModel::$user_profile->getUuid();
            }
            if (count($viewers)) {
        ...
        if ($object_uuid != '' && Helpers::__isValidUuid($object_uuid)) {
            ...
            if (count($membersUuids)) {
                foreach ($membersUuids as $user_profile_uuid) {
                     if ($user_profile_uuid != ModuleModel::$user_profile->getUuid()) {
                        $channels[] = $user_profile_uuid;
                    }
                };
                if($data->action == 'HISTORY_REMOVE' && $data->type == 'S'){
                    $channels[] = ModuleModel::$user_profile->getUuid();
                }
    ...
    public function getSimpleListNotificationAction($time = '')
    {
        ...
        $params = [
            ...
            'isAdminOrManager' => ModuleModel::$user_profile->isAdminOrManager(),
    ...
    {
        $this->checkAjaxGet();
        $user_profile_data = ModuleModel::$user_profile->getUserProfileData();
        ...
        $return['profile'] = ModuleModel::$user_profile->getUuid();
...
//            $notificationObject = RelodayDynamoORM::factory('\Reloday\Application\Models\RelodayHistory')
...
//                ->where('created_at', '<', strval($time));
    ...
    public function getNotificationHistoryTodayByUser($user_profile_uuid)
    {
        ...
        $notificationObject = RelodayDynamoORM::factory('\Reloday\Application\Models\RelodayHistoryNotification')
    ...
    public function getLastNotificationAction()
    {
        ...
        try {
            ...
            $modelHistoryNotificationObject = DynamoORM::factory('\Reloday\Application\Models\RelodayHistoryNotification')
    ...
    public function addNotificationForTagUserAction()
    {
        ...
        if ($object_uuid != '' && Helpers::__isValidUuid($object_uuid)) {
            ...
            if($targetUser){
                ...
                if ($profile && $profile->getUuid() != ModuleModel::$user_profile->getUuid()) {

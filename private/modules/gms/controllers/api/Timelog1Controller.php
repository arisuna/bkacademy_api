 ...
 * @RoutePrefix("/gms/api")
 */
class Timelog1Controller extends BaseController
{
    ...
    public function getListAction()
    {
        $this->view->disable();
        $this->checkAjaxPost();
        $this->checkAclIndex(AclHelper::CONTROLLER_TIMELOG);
...
//            $total_user = $queryBuilder->getQuery()->execute();
    ...
    public function createTimelogAction()
    {
        $this->view->disable();
        $this->checkAjaxPost();
        $this->checkAclCreate(AclHelper::CONTROLLER_TIMELOG);
        ...
        if ($relocation->getStatus() != Relocation::STATUS_ONGOING) {
            $result = [
                "success" => false,
                "message" => "RELOCATION_STATUS_NOT_STARTED_TEXT",
                "detail" => "relocation is not in progress"
            ];
            goto end_of_function;
        }
        ...
        $task = Task::findFirstByUuid($data['task_uuid']);
        if (!$task || $task->getStatus() != Task::STATUS_ACTIVE) {
            $result = [
                "success" => false,
                "message" => "SAVE_TIMELOG_FAIL_TEXT",
                "detail" => "task is not activated"
            ];
            goto end_of_function;
        }
    ...
    public function editTimelogAction()
    {
        $this->view->disable();
        $this->checkAjaxPost();
        $this->checkAclEdit(AclHelper::CONTROLLER_TIMELOG);
        ...
        if ($uuid != '' && Helpers::__isValidUuid($uuid)) {
            ...
            if ($timelog instanceof Timelog && $timelog->belongsToGms()) {
                ...
                if ($result['success'] == false) {
                    $result['message'] = 'SAVE_TIMELOG_FAIL_TEXT';
                } else $result['message'] = 'SAVE_TIMELOG_SUCCESS_TEXT';
    ...
    public function removeTimelogAction($uuid)
    {
        ...
        $this->checkAclDelete(AclHelper::CONTROLLER_TIMELOG);
